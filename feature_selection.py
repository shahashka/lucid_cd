...
PHENOTYPE_MODEL = { "elastic_multitask" : MultiTaskElasticNet(alpha=0.1, l1_ratio=0.5, max_iter=30000)}
...
def model_fit(X,y, model, genes=None):
    if genes is not None:
        genes_filtered = list(set(genes).intersection(set(X.columns)))
        X = X[genes_filtered]
    if model == PHENOTYPE_MODEL['elastic_multitask']:
...
def model_eval(model, X_eval, y_eval):
    return model.score(X_eval, y_eval)
...
def load_data(args):
    ...
    log2fold_df['dose_rate'] = [DOSE_RATES[d[1]] for d in log2fold_df['dose_rate']]
    ...
    return X, labels_dr, labels_w, phenotypes.to_numpy()
...
def recursive_features(X, y, model, min_features=1000):
    X = _remove_labels(X)
    selector = RFECV(model, step=100, cv=5, min_features_to_select=min_features)
...
def plot_scores(all_scores):
    ...
    for model_name, model_scores in all_scores.items():
        ...
        ax.set_xlabel("Week accuracy")
        ax.set_ylabel("Dose rate accuracy")
        ax.set_title(f"{model_name} Pareto front (feature sets)")
...
GENE_RANGE = [10, 20, 30, 50, 100, 150, 200]
...
def run_increasing_num_genes(X, labels, phenotypes):
    """
    ...
    for num_genes in GENE_RANGE:
        for method in INCREASING_NUM_GENES_METHODS:
            for model_name, model in MODELS.items():
                for label_name, y in labels.items():
                    try:
                        ...
                        continue
                    mean_acc, std_acc = model_fit(X, y, clone(model), genes=genes)
                    rows.append({
                        "num_genes": num_genes,
                    ...
                    print(
                        ...
                        f"accuracy={mean_acc:.3f} ± {std_acc:.3f} (n_genes={len(genes)})"
                    )
            mean_acc, std_acc = model_fit(X, phenotypes, PHENOTYPE_MODEL["elastic_multitask"], genes=genes)
            rows.append({
                "num_genes": num_genes,
                "feature_selection": method,
                "model": "elastic_multitask",
                "label": 'phenotypes',
                "accuracy_mean": float(mean_acc),
                "accuracy_std": float(std_acc),
                "n_genes_used": len(genes),
            ...
            print(
                f"num_genes={num_genes} | {method} | elastic_multitask | phenotypes: "
                f"accuracy={mean_acc:.3f} ± {std_acc:.3f} (n_genes={len(genes)})"
            )
    ...
    for model_name in models:
        for label_name in labels:
            ...
            for method in methods:
                ...
                y = m["accuracy_mean"].values
                yerr = m["accuracy_std"].values
                # ax.errorbar(
                #     x, y, yerr=yerr,
                #     label=method, capsize=3, capthick=1, marker="o", markersize=4,
                # )
                ax.plot(
                    x, y,
            ...
            ax.set_ylim(0, 1.05)
            plt.tight_layout()
            safe_name = f"{out_prefix}_{model_name}_{label_name}.png"
...
if __name__ == "__main__":
        ...
        for model_name, model in MODELS.items():
            ...
            for label_name, y in labels.items():
                ...
                for feat_name, genes in feature_sets.items():
                    ...
                    mean, std = model_fit(X, y, model, genes_filtered)
    ...
    if args.run_increasing_genes:
        ...
        df_rig = run_increasing_num_genes(X,labels, phenotypes)
